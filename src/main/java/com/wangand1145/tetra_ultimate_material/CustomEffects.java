...
public class CustomEffects {
    ...
    public static final ItemEffect TORCH_CRAFT = ItemEffect.get("tetra_ultimate_material.torch_craft");
    ...
    public static void onBlockBreak(BlockEvent.BreakEvent event) {
        ...
        if (event.getState().getBlock() != Blocks.COAL_ORE && event.getState().getBlock() != Blocks.DEEPSLATE_COAL_ORE) return;

        if (player.level().random.nextFloat() >= 0.025f * level / 10f) return;
    ...
    public static void onCraft(PlayerEvent.ItemCraftedEvent event) {
        ...
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ...
            if (slot.getItem() instanceof ModularItem) {
                tool = slot;
                break;
            }
        }
